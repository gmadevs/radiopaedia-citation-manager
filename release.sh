...
cd "$(dirname "$0")"
SCRIPT=radiopaedia-cite.user.js
...
# The tests are the only thing standing between a bad regex and somebody's
# article, and they take four seconds.
if ! ./tools/test/run.sh; then
  echo "tests failed — nothing released" >&2
  exit 1
fi
